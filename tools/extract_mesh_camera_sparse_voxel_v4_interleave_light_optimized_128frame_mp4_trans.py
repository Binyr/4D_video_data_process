...
- Replace per-frame bbox-center translation removal with per-frame inverse rigid transform
  (estimated by Kabsch) to a canonical reference frame.
"""
...
def transform_points_with_rigid_matrix(points: np.ndarray, rigid_4x4: np.ndarray):
    """
    points: [N, 3], row-vector convention
    rigid_4x4 maps current world vertices -> canonical vertices
    """
    R = np.asarray(rigid_4x4[:3, :3], dtype=np.float32)
    t = np.asarray(rigid_4x4[:3, 3], dtype=np.float32)
    return points @ R.T + t[None, :]


def estimate_rigid_transform_kabsch(src_points: np.ndarray, dst_points: np.ndarray):
    """
    Estimate rigid transform M such that:
        dst ~= src @ R^T + t
    ...
    if src.shape != dst.shape or src.ndim != 2 or src.shape[1] != 3:
        raise ValueError(
            f"Invalid shape for rigid estimation: src={src.shape}, dst={dst.shape}"
        )
...
def make_scaled_transform_matrix(rigid_4x4: np.ndarray, global_scale: float):
    """
    Final normalizer matrix = Scale(global_scale) @ rigid_transform
    """
    S = np.eye(4, dtype=np.float32)
    S[0, 0] = float(global_scale)
    S[1, 1] = float(global_scale)
    S[2, 2] = float(global_scale)
    return S @ np.asarray(rigid_4x4, dtype=np.float32)


def set_object_from_matrix_world_np(obj, matrix_world_np: np.ndarray):
    obj.rotation_mode = "QUATERNION"
    M = Matrix(np.asarray(matrix_world_np, dtype=np.float32).tolist())
...
def create_sequence_normalizer():
    ...
    normalizer.rotation_mode = "QUATERNION"
    normalizer.rotation_quaternion = (1.0, 0.0, 0.0, 0.0)
...
def compute_sequence_normalization_params_and_cache(
    ...
    First pass only:
      ...
      2) compute per-frame bbox centers (debug / metadata only)
      3) estimate per-frame rigid transform to a canonical reference frame
      4) compute canonical bbox after inverse rigid transform
    ...
    ref_vertices_raw = None
    ref_vertices_canonical = None
    ...
    for i in frame_indices:
        ...
        if ref_vertices_raw is None:
            reference_frame = int(i)
            ref_vertices_raw = raw_vertices.astype(np.float32, copy=True)

            # reference frame is only centered once to define canonical coordinates
            ref_vertices_canonical = ref_vertices_raw - center[None, :]

            rigid_M = np.eye(4, dtype=np.float32)
            rigid_M[:3, 3] = -center.astype(np.float32)
            aligned = ref_vertices_canonical

            print(f"Canonical reference frame set to frame {reference_frame}")
        else:
            rigid_M = estimate_rigid_transform_kabsch(
                src_points=raw_vertices.astype(np.float32, copy=False),
                dst_points=ref_vertices_canonical,
            )
            aligned = transform_points_with_rigid_matrix(raw_vertices, rigid_M)

        rigid_matrices_seq.append(rigid_M.astype(np.float32))

        global_min = np.minimum(global_min, aligned.min(axis=0))
        global_max = np.maximum(global_max, aligned.max(axis=0))

        if cache_raw_vertices:
            raw_vertices_cache.append(raw_vertices.astype(raw_cache_dtype))

    centers_seq = np.stack(centers_seq, axis=0).astype(np.float32)
    rigid_matrices_seq = np.stack(rigid_matrices_seq, axis=0).astype(np.float32)
...
def apply_sequence_normalization(normalizer_obj, frame_rigid_matrix: np.ndarray, global_scale: float):
    M = make_scaled_transform_matrix(frame_rigid_matrix, global_scale)
    set_object_from_matrix_world_np(normalizer_obj, M)


def bake_sequence_normalization_to_keyframes(
    normalizer_obj,
    frame_indices: np.ndarray,
    rigid_matrices_seq: np.ndarray,
    global_scale: float,
):
    scene = bpy.context.scene
    ...
    normalizer_obj.rotation_mode = "QUATERNION"

    for local_frame_idx, source_frame in enumerate(frame_indices):
        scene.frame_set(int(source_frame))
        M = make_scaled_transform_matrix(rigid_matrices_seq[local_frame_idx], global_scale)
        set_object_from_matrix_world_np(normalizer_obj, M)
        normalizer_obj.keyframe_insert(data_path="location", frame=int(source_frame))
        normalizer_obj.keyframe_insert(data_path="rotation_quaternion", frame=int(source_frame))
...
def precompute_normalized_mesh_sequence_from_cache(
    ...
    for local_frame_idx, raw_vertices in enumerate(tqdm(raw_vertices_cache, desc="Normalizing cached mesh sequence")):
        ...
        norm_vertices = transform_points_with_rigid_matrix(
            raw32,
            rigid_matrices_seq[local_frame_idx],
        )
        norm_vertices = (norm_vertices * global_scale).astype(np.float32)
...
def process_geometry(args):
    ...
    timer.log("first pass: topology + inverse-rigid normalization params + raw cache")

    print("Sequence normalization:")
    print(f"  sequence_scale = {sequence_scale}")
    print(f"  canonical_reference_frame = {reference_frame}")
    ...
    vertices_seq, keyframe_ply_records = precompute_normalized_mesh_sequence_from_cache(
        ...
        rigid_matrices_seq=rigid_matrices_seq,
    ...
    for local_frame_idx, source_frame in enumerate(tqdm(frame_indices, desc="Rendering frames")):
        ...
        apply_sequence_normalization(
            sequence_normalizer,
            frame_rigid_matrix=rigid_matrices_seq[local_frame_idx],
            global_scale=sequence_scale,
        )
        bpy.context.view_layer.update()
        ...
        for view_idx, (camera_obj, camera_info) in enumerate(zip(camera_objs, camera_infos)):
            ...
            camera_meta = {
                ...
                "frame_rigid_normalization_matrix": rigid_matrices_seq[local_frame_idx].astype(np.float32).tolist(),
    ...
    final_watertight_data["_global"] = {
        ...
        "root_body_motion_removed_by": "per-frame inverse rigid transform to canonical reference frame",
        "normalization_reference_frame": int(reference_frame),
